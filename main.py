...
def main() -> int:
    ...
    _server_thread = run_server_in_thread(
        ...
        store=store,
        quality_clips_dir=str(getattr(cfg, "recording_assets_dir", "./assets")) + "/quality_clips",
    ...
    params = DetectionParams(
        ...
        blockiness_enabled=bool(getattr(cfg, "blockiness_enabled", True)),
        blockiness_block_sizes=list(getattr(cfg, "blockiness_block_sizes", [8, 16])),
        blockiness_sample_every_frames=int(getattr(cfg, "blockiness_sample_every_frames", 25)),
        blockiness_downscale_width=int(getattr(cfg, "blockiness_downscale_width", 640)),
        blockiness_ema_alpha=float(getattr(cfg, "blockiness_ema_alpha", 0.25)),
        quality_enabled=bool(getattr(cfg, "quality_enabled", True)),
        quality_sample_every_frames=int(getattr(cfg, "quality_sample_every_frames", 3)),
        quality_downscale_width=int(getattr(cfg, "quality_downscale_width", 640)),
        quality_threshold_ringing=float(getattr(cfg, "quality_threshold_ringing", 0.65)),
        quality_threshold_banding=float(getattr(cfg, "quality_threshold_banding", 0.65)),
        quality_threshold_cadence_jitter=float(getattr(cfg, "quality_threshold_cadence_jitter", 0.65)),
        quality_threshold_duplicate_ratio=float(getattr(cfg, "quality_threshold_duplicate_ratio", 0.65)),
        quality_threshold_motion_blur=float(getattr(cfg, "quality_threshold_motion_blur", 0.65)),
        quality_trigger_consecutive_samples=int(getattr(cfg, "quality_trigger_consecutive_samples", 3)),
        quality_trigger_cooldown_seconds=float(getattr(cfg, "quality_trigger_cooldown_seconds", 20.0)),
        quality_recording_enabled=bool(getattr(cfg, "quality_recording_enabled", True)),
        quality_pre_roll_seconds=float(getattr(cfg, "quality_pre_roll_seconds", 2.0)),
        quality_post_roll_seconds=float(getattr(cfg, "quality_post_roll_seconds", 2.0)),
        quality_max_clip_seconds=float(getattr(cfg, "quality_max_clip_seconds", 20.0)),
        quality_recording_cooldown_seconds=float(getattr(cfg, "quality_recording_cooldown_seconds", 20.0)),
